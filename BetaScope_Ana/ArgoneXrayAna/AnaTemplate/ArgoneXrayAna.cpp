...
void ArgoneXrayAna::Initialize() {
  ...
  if (check_path != NULL) {
    ...
    BetaScope_AnaFramework::Initialize(
        beta_scope_path +
        "/../BetaScope_Ana/ArgoneXrayAna/AnaTemplate/myOwnTree.ini");
  } else {
    BetaScope_AnaFramework::Initialize("");
  }
  ...
  for (int ch = 0; ch < 16; ch++) {
    ...
    if (ch == 0) {
      br_check =
          this->beta_scope.SetInBranch<TTreeReaderArray, double>("t", "t");
  ...
  */
  ...
  for (int i = 0; i < 16; i++) {
    auto br_check =
        this->beta_scope.BuildBranch<std::vector<double>>(Form("pmax%i", i));
    br_check =
        this->beta_scope.BuildOutBranch<std::vector<double>>(Form("tmax%i", i));
...
void ArgoneXrayAna::LoopEvents() {
  ...
  while (this->beta_scope.GetInTreeReader()->Next()) {
    ...
    ArgoneXrayAna::copyTTreeReaderArrayToVector<std::vector<double>, double>,
