...
void BetaScopeWaveformAna::event_ana(int ch) {
  ...
  WaveformAna<double, double> waveform = WaveAna.analyze_waveform(
      this->t[ch], this->w[ch], this->my_anaParam.limiting_search_region_OnOff,
      this->my_anaParam.pmaxSearchRange);

  this->frontBaselineInt_indepBaseCorr[ch]->push_back(
      waveform.get_front_baseline_int());
  this->backBaselineInt_indepBaseCorr[ch]->push_back(
      waveform.get_back_baseline_int());
  ...
  this->pulseArea_withUndershoot[ch]->push_back(
      waveform.get_pulse_area_undershoot());
  ...
  */
  ...
  ///*
  for (auto value : waveform.get_cfd()) {
    this->cfd[ch]->push_back(value);
  }
  for (auto value : waveform.get_cfd_fall()) {
    this->cfd_fall[ch]->push_back(value);
  }
  for (auto value : waveform.get_dvdt()) {
    this->dvdt[ch]->push_back(value);
  }
  for (auto value : waveform.get_threshold_time()) {
    this->thTime[ch]->push_back(value);
  }
  for (auto value : waveform.get_fine_cfd()) {
    this->fineCFDRise[ch]->push_back(value);
  }
  //*/
...
void BetaScopeWaveformAna::Analysis() {
      ...
      if (true) {
        ...
        for (std::size_t i = 0, max = this->i_w[ch]->GetSize(); i < max; i++) {
          ...
          if (this->invertChannels.at(chh) != 0) {
            this->w[ch]->push_back(this->i_w[ch]->At(i) * -1.0 *
                                   this->voltageMultiFactor);
            // this->w[ch]->at(i) = this->i_w[ch]->At(i) * -1.0 *
            // this->voltageMultiFactor;
          } else {
            this->w[ch]->push_back(this->i_w[ch]->At(i) *
                                   this->voltageMultiFactor);
          ...
          if (this->resample_time) {
            this->t[ch]->push_back((this->xorigin + i * this->dt) *
                                   this->timeMultiFactor);
            // this->xorigin = this->xorigin + this->dt;
          } else {
            this->t[ch]->push_back(this->i_t[ch]->At(i) *
                                   this->timeMultiFactor);
          }
        ...
        // this, ch) );
        workers.push_back(
            std::async(&BetaScopeWaveformAna::event_ana, this, ch));
      }
    }
  }
  ...
  if (this->has_daq_cycle) {
    this->beta_scope.SetOutBranchValue(
        "cycle",
        this->beta_scope.GetInBranchValue<TTreeReaderValue, int>("cycle"));
  }
  if (this->has_daq_temperature) {
    this->beta_scope.SetOutBranchValue(
        "temperature",
        this->beta_scope.GetInBranchValue<TTreeReaderValue, double>(
            "temperature"));
  }
  if (this->has_daq_humidity) {
    this->beta_scope.SetOutBranchValue(
        "humidity", this->beta_scope.GetInBranchValue<TTreeReaderValue, double>(
                        "humidity"));
  }
  if (this->has_daq_bias) {
    this->beta_scope.SetOutBranchValue(
        "bias",
        this->beta_scope.GetInBranchValue<TTreeReaderValue, double>("bias"));
  }
  ...
  if (this->has_daq_current) {
    this->beta_scope.SetOutBranchValue(
        "current",
        this->beta_scope.GetInBranchValue<TTreeReaderValue, double>("current"));
  }
  if (this->has_daq_eventNum) {
    this->beta_scope.SetOutBranchValue(
        "ievent",
        this->beta_scope.GetInBranchValue<TTreeReaderValue, int>("ievent"));
  }
}
...
void BetaScopeWaveformAna::Initialize() {
  ...
  if (!this->skipWaveform) {
    for (auto ch : this->beta_scope.channel) {
      ...
      ColorCout::print(
          "  ", "Creating branches for storing scope channels: ", YELLOW);
      branch_checker =
          this->beta_scope.BuildOutBranch<std::vector<double>>(Form("w%i", ch));

      branch_checker =
          this->beta_scope.BuildOutBranch<std::vector<double>>(Form("t%i", ch));
  ...
  for (int chh = 0; chh < this->activeChannels.size(); chh++) {
    ...
    if (!this->skipWaveform) {
      this->w[ch] = this->beta_scope.GetOutBranch<std::vector<double>>(
          "w" + std::to_string(ch));
      this->t[ch] = this->beta_scope.GetOutBranch<std::vector<double>>(
          "t" + std::to_string(ch));
    } else {
    ...
    this->pmax[ch] = this->beta_scope.GetOutBranch<std::vector<double>>(
        "pmax" + std::to_string(ch));
    this->tmax[ch] = this->beta_scope.GetOutBranch<std::vector<double>>(
        "tmax" + std::to_string(ch));
    this->neg_pmax[ch] = this->beta_scope.GetOutBranch<std::vector<double>>(
        "neg_pmax" + std::to_string(ch));
    this->neg_tmax[ch] = this->beta_scope.GetOutBranch<std::vector<double>>(
        "neg_tmax" + std::to_string(ch));

    this->riseTime[ch] = this->beta_scope.GetOutBranch<std::vector<double>>(
        "riseTime" + std::to_string(ch));
    this->dvdt[ch] = this->beta_scope.GetOutBranch<std::vector<double>>(
        "dvdt" + std::to_string(ch));
    this->cfd_fall[ch] = this->beta_scope.GetOutBranch<std::vector<double>>(
        "cfd_fall" + std::to_string(ch));
    this->cfd[ch] = this->beta_scope.GetOutBranch<std::vector<double>>(
        "cfd" + std::to_string(ch));
    this->fineCFDRise[ch] = this->beta_scope.GetOutBranch<std::vector<double>>(
        "fineCFDRise" + std::to_string(ch));
    this->thTime[ch] = this->beta_scope.GetOutBranch<std::vector<double>>(
        "thTime" + std::to_string(ch));
    this->rms[ch] = this->beta_scope.GetOutBranch<std::vector<double>>(
        "rms" + std::to_string(ch));
    this->pulseArea_withUndershoot[ch] =
        this->beta_scope.GetOutBranch<std::vector<double>>(
            "pulseArea_withUndershoot" + std::to_string(ch));
    this->pulseArea_withZeroCross[ch] =
        this->beta_scope.GetOutBranch<std::vector<double>>(
            "pulseArea_withZeroCross" + std::to_string(ch));
    this->frontBaselineInt_indepBaseCorr[ch] =
        this->beta_scope.GetOutBranch<std::vector<double>>(
            "frontBaselineInt_indepBaseCorr" + std::to_string(ch));
    this->backBaselineInt_indepBaseCorr[ch] =
        this->beta_scope.GetOutBranch<std::vector<double>>(
            "backBaselineInt_indepBaseCorr" + std::to_string(ch));

    this->i_w[ch] = this->beta_scope.GetInBranch<TTreeReaderArray, double>(
        "w" + std::to_string(ch));
    this->i_t[ch] = this->beta_scope.GetInBranch<TTreeReaderArray, double>(
        "t" + std::to_string(ch));
    if (!this->i_w[ch]) {
      std::cout << this->i_w[ch] << std::endl;
      std::cout << this->beta_scope.GetInBranch<TTreeReaderArray, double>(
                       "w" + std::to_string(ch))
                << std::endl;
    }
    this->beta_scope.BuildOutBranch<std::vector<WaveformAna<double, double>>>(
        "waveform" + std::to_string(ch));
    this->waveform_ana[ch] =
        this->beta_scope.GetOutBranch<std::vector<WaveformAna<double, double>>>(
            "waveform" + std::to_string(ch));
  }
