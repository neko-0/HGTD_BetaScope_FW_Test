...
def plotHisto(
    ...
    for runIndex in range(int(num_file)):
        ...
        try:
            temperature = (
                config_file["run%s" % runIndex]["file_name"]
                .split("temp")[1]
                .split(".root")[0]
            )
        except:
        ...
        if (
            volt_min < bias_value
            and bias_value < volt_max
            and temperature == set_temperature
        ):
            ...
            if binSize < 0:
                histo1D = ROOT.TH1D(
                    "bias", "Sensor:%s Param:%s" % (sensor_name, param), 200, 1, 1
                )
            else:
                histo1D = ROOT.TH1D(
                    "bias",
                    "Sensor:%s Param:%s" % (sensor_name, param),
                    binNum,
                    low_b,
                    high_b,
                )
...
if __name__ == "__main__":
    ...
    commandline_ArgsParser.add_argument(
        "--low", dest="low_b", nargs="?", default="0", type=int, help="low bound"
    )
    commandline_ArgsParser.add_argument(
        "--high", dest="high_b", nargs="?", default="500", type=int, help="high bound"
    )
    commandline_ArgsParser.add_argument(
        "--param",
        dest="param",
    ...
    commandline_ArgsParser.add_argument(
        "--xtitle", dest="xtitle", nargs="?", default="", type=str, help="x axis tilte"
    )
    commandline_ArgsParser.add_argument(
        "--temp", dest="temp", nargs="?", default="-30", type=str, help="temperature"
    )
    commandline_ArgsParser.add_argument(
        "--configFile",
        dest="configFile",
    ...
    for stuff in histoList:
        ...
        fit = ROOT.TF1(
            "landau",
            "landau",
            stuff[2].GetMean() - stuff[2].GetStdDev(),
            stuff[2].GetMean() + 5 * stuff[2].GetStdDev(),
        )
        stuff[2].Fit(fit, "R")
        ...
        low_50percent_bin = (
            stuff[2].GetXaxis().GetBinCenter(stuff[2].FindFirstBinAbove(maxValue / 2))
        )
        high_50percent_bin = (
            stuff[2].GetXaxis().GetBinCenter(stuff[2].FindLastBinAbove(maxValue / 2))
        )
