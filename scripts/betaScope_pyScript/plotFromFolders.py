...
for parv in pars:
    ...
    if parv[0].split(":")[1] == "V":
        ...
        for folder in dirlist:
            ...
            markstyle = 20
            if "1E14" in fold or "1e14" in fold:
                markstyle = 21
            if "4E14" in fold or "4e14" in fold:
                markstyle = 22
            if "8E14" in fold or "8e14" in fold:
                markstyle = 23
            if "1p5E15" in fold or "1p5e15" in fold:
                markstyle = 24
            if "2p25E15" in fold or "2p25e15" in fold:
                markstyle = 25
            if "3E15" in fold or "3e15" in fold:
                markstyle = 26
            if "4E15" in fold or "4e15" in fold:
                markstyle = 27
            if "_5E15" in fold or "_5e15" in fold:
                markstyle = 28
            if "6E15" in fold or "6e15" in fold:
                markstyle = 29
            ...
            for col, run in enumerate(runlist):
                if "Run" + run in fold:
                    ...
                    for ch in dut_trig:
                        for bias_run in config_section:
                            if ch in bias_run:
                                if ch != "Trig":
                                    biasV = bias_run[
                                        bias_run.find("_") + 1 : bias_run.find("V")
                                    ]
                    ...
                    if maxval < max(values):
                        maxval = max(values)
                    if maxvol < max(bias_voltages):
                        maxvol = max(bias_voltages)
        ...
        h.GetXaxis().SetRangeUser(0.0, maxvol * 1.2)
        h.GetYaxis().SetRangeUser(0, maxval * 2.0)
