...
def ParseINItoROOT(fname="_results.ini"):
    ...
    for ch in dut_trig:
        ...
        ttree = ROOT.TTree(f"run{description_file.run_number}", "from _results.ini")
        for par in INI_TO_EXCEL.keys():
            if "SensorName" in par:
                branches[par] = array("b")
                branches[par].frombytes(sensor_name.encode())
                ttree.Branch(par, branches[par], f"{par}/C")
            elif "runNumber" in par:
                branches[par] = array("i", [0])
                ttree.Branch(par, branches[par], f"{par}/I")
            else:
        ...
        ttree.Write(f"run{description_file.run_number}", ROOT.TObject.kOverwrite)
        tfile.Close()
