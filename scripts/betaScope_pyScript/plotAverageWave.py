...
def plot_average_waveform( config, volt_min=0, volt_max=1000, norm=0):
    ...
    for runIndex in range(int(num_file)):
        ...
        if volt_min < bias_value and bias_value < volt_max:
            ...
            if True:
                ...
                for k in range(0,maxValueIndex):
                    if maxValue/2.0 == abs(ave.GetBinContent(k)):
                        ...
                        slope = float((abs(ave.GetBinContent(k))-abs(ave.GetBinContent(k-1))))/(ave.GetBinCenter(k)-ave.GetBinCenter(k-1))
                        intercept = abs(ave.GetBinContent(k))-slope*ave.GetBinCenter(k)
                ...
                for k in range(maxValueIndex, 10000):
                    if maxValue/2.0 == abs(ave.GetBinContent(k)):
                        ...
                        slope = (abs(float(ave.GetBinContent(k-1)))-abs(ave.GetBinContent(k)))/(ave.GetBinCenter(k-1)-ave.GetBinCenter(k))
                        intercept = abs(ave.GetBinContent(k))-slope*ave.GetBinCenter(k)
                        right50 = (maxValue/2.0 - intercept)/float(slope)
                        break
                    else: continue
                if norm!=0:ave.Scale(1.0/maxValue)
            ...
            color+=1
            avelist.append( {"sensor":sensor_name, "bias":bias, "wf":ave, "max":maxValue, "left50":left50, "right50":right50, "color":color-1, "lline":lline, "rline":rline} )
        else:
            continue
...
if __name__ == "__main__":
    ...
    commandline_ArgsParser.add_argument("--norm", dest="norm", nargs="?", default="1", type=int, help="Normalized waveform")
