...
def generate_cuts(
    ...
    for i in range(num_file):
        ...
        ttree.Project(
            "histo",
            tmax_expr,
            f"{tmax_expr}>{tmax_min_bound} && {tmax_expr}<{tmax_max_bound}",
        )
        tmax = ROOT.RooRealVar("tmax", "tmax", -2000, 2000)
        tmax_mean = ROOT.RooRealVar("mean", "guassian mean", -1500, 1500)
        tmax_width = ROOT.RooRealVar("sigma", "guassian width", 1, 1000)
        Roohisto = ROOT.RooDataHist(
            "Roohisto", "Tmax distribution", ROOT.RooArgList(tmax), histo
        )
        gaussian = ROOT.RooGaussian(
            "gaussian", "guassian for signal", tmax, tmax_mean, tmax_width
        )
        poly0 = ROOT.RooPolynomial("poly0", "poly0", tmax)
        gauss_coeff = ROOT.RooRealVar("gauss_coeff", "gaussian yiled", 0.5, 0, 1)
        tmaxModel = ROOT.RooAddPdf(
            "tmaxModel",
            "Tmax Fit Model",
            ROOT.RooArgList(gaussian, poly0),
            ROOT.RooArgList(gauss_coeff),
        )
        frame = tmax.frame()
        tmaxModel.fitTo(Roohisto)
        Roohisto.plotOn(frame)
        ...
        tCut = f"{fitted_mean-numOfSigma*fitted_sigma} {fitted_mean+numOfSigma*fitted_sigma}"
        print(fitted_sigma)
        ...
        pmaxSignalHisto = ROOT.TH1D(
            "pmaxSignalHisto", "Pmax Distribution in Signal Region", 250, -50, 360
        )
        ttree.Project(
            "pmaxSignalHisto",
            pmax_expr,
            f"{fitted_mean-numOfSigma*fitted_sigma} < {tmax_expr} && {tmax_expr} < {fitted_mean+numOfSigma*fitted_sigma}",
        )
        RoopmaxSignalHisto = ROOT.RooDataHist(
            "RoopmaxSignalHisto",
            "Pmax distribution in Singal",
            ROOT.RooArgList(pmaxSignal),
            pmaxSignalHisto,
        )
        landauDistri = ROOT.RooLandau(
            "landauDistri", "landau distribution", pmaxSignal, landauMean, landauSigma
        )
        gaussNoise_mean = ROOT.RooRealVar(
            "gaussNoise_mean", "gaussNoise_mean", 50, -50, 360
        )
        gaussNoise_sigma = ROOT.RooRealVar(
            "gaussNoise_sigma", "gaussNoise_sigma", 50, -50, 360
        )
        gaussianNoise = ROOT.RooGaussian(
            "gaussianNoise",
            "gaussianNoise",
            pmaxSignal,
            gaussNoise_mean,
        ...
        signal_plus_bkg = ROOT.RooFFTConvPdf(
            "lxg", "landau (X) gauss", pmaxSignal, landauDistri, gaussianNoise
        )
        ...
        pmaxSidebandHisto = ROOT.TH1D(
            "pmaxSidebandHisto", "Pmax Distribution in sideband Region", 200, -80, 80
        )
        ttree.Project(
            "pmaxSidebandHisto",
            pmax_expr,
            "{tmax_expr} < {fitted_mean-numOfSigma*fitted_sigma} || {tmax_expr} > {fitted_mean-numOfSigma*fitted_sigma}",
        )
        RoopmaxSidebandHisto = ROOT.RooDataHist(
            "RoopmaxSidebandHisto",
            "Pmax distribution in sideband",
            ROOT.RooArgList(pmaxNoise),
            pmaxSidebandHisto,
        )
        sideband_gaussNoise_mean = ROOT.RooRealVar(
            "sideband_gaussNoise_mean", "sideband_gaussNoise_mean", 50, -80, 80
        )
        sideband_gaussNoise_sigma = ROOT.RooRealVar(
            "sideband_gaussNoise_sigma", "sideband_gaussNoise_sigma", 50, 1, 80
        )
        sideband_gaussianNoise = ROOT.RooGaussian(
            "sideband_gaussianNoise",
            "sideband_gaussianNoise",
            pmaxNoise,
            sideband_gaussNoise_mean,
            sideband_gaussNoise_sigma,
        )
        ...
        for ientry, entry in enumerate(ttree):
            x.append(getattr(entry, f"pmax{dutCh}")[0])
            y.append(
                getattr(entry, f"tmax{dutCh}")[0] - getattr(entry, f"cfd{trigCh}")[20]
            )
            if ientry % 1000 == 0:
                print(f"@ event {ientry}")
            for item in threshold:
                if (
                    fitted_mean - numOfSigma * fitted_sigma < y[ientry]
                    and y[ientry] < fitted_mean + numOfSigma * fitted_sigma
                    and x[ientry] > item[0]
                ):
                    item[1] += 1
        graph = ROOT.TGraph(len(x), x, y)
        graph.GetXaxis().SetTitle(f"Pmax_ch{dutCh}[mV]")
        graph.GetYaxis().SetTitle(f"Tmax_ch{dutCh}-TrigCFD_ch{trigCh}[ps]")
        graph.GetYaxis().SetRangeUser(
            fitted_mean - 7.0 * fitted_sigma, fitted_mean + 7.0 * fitted_sigma
        )
        graph.GetXaxis().SetRangeUser(0, 5 * pmax_peak)
        tCutLine_min = ROOT.TLine(
            0,
            fitted_mean - numOfSigma * fitted_sigma,
            graph.GetXaxis().GetXmax(),
            fitted_mean - numOfSigma * fitted_sigma,
        )
        tCutLine_max = ROOT.TLine(
            0,
            fitted_mean + numOfSigma * fitted_sigma,
            graph.GetXaxis().GetXmax(),
            fitted_mean + numOfSigma * fitted_sigma,
        ...
        if find_good_threshold:
            pmaxCutLine_min = ROOT.TLine(
                apply_th_cut,
                graph.GetYaxis().GetXmin(),
                apply_th_cut,
                graph.GetYaxis().GetXmax(),
            )
            pmaxCutLine_max = ROOT.TLine(
                360, graph.GetYaxis().GetXmin(), 360, graph.GetYaxis().GetXmax()
            )
            pmaxCut = f"{apply_th_cut} 360"
        else:
            pmaxCutLine_min = ROOT.TLine(
                typical_noise,
                graph.GetYaxis().GetXmin(),
        ...
        with open(f"{configFileName}", "w") as configOutput:
...
if __name__ == "__main__":
    ...
    commandline_ArgsParser.add_argument(
        "--dutCh", dest="dutCh", nargs="?", default=2, type=str, help="DUT Channel"
    )
    commandline_ArgsParser.add_argument(
        "--trigCh",
        dest="trigCh",
    ...
    commandline_ArgsParser.add_argument(
        "--runNum", dest="runNum", type=int, help="Run number"
    )
    commandline_ArgsParser.add_argument(
        "--configFile",
        dest="configFile",
    ...
    commandline_ArgsParser.add_argument(
        ...
        dest="trigFixedCut",
        nargs="?",
        default="200 500 70 360",
        type=str,
        help="Preset trigger cut",
    )
    commandline_ArgsParser.add_argument(
        "--tmax_minB",
        dest="tmax_minB",
