...
PlotConfigMgr::PlotConfigMgr( std::string ifile )
{
  ...
  this->average_pulse_ofile_name = pt.get<std::string>("header.average_pulse_ofile_name");
  ...
  for( std::size_t i = 0, max = this->number_of_runs; i < max; i++ )
  {
    ...
    if(this->use_selected_events){
      this->file_name[i].push_back( "Selected_"+pt.get<std::string>(file_name) );
    }
    else{
...
void PlotConfigMgr::PrintContent()
{
  ...
  std::cout << "average_pulse_ofile_name= " << this->average_pulse_ofile_name << std::endl;
