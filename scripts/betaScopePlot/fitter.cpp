...
FitResult Fitter::fitter_RooLanGausArea(HistoPackage &i_histo,
  ...
  RooRealVar x("x", i_histo.get_observable().c_str(), i_histo.get_xmin(),
               i_histo.get_xmax());
  ...
  double histo_max = i_histo.get_histo()->GetXaxis()->GetBinCenter(
      i_histo.get_histo()->GetMaximumBin());
  ...
  baseline_area_mean =
      (backBaselineFitter->GetParameter(1) + baseline_area_mean) /
      2.0; // std::cout << baseline_area_mean << std::endl; std::cin.get();
  baseline_area_sigma =
      (backBaselineFitter->GetParameter(2) + baseline_area_sigma) / 2.0;
  baseline_area_sigma_error =
      (backBaselineFitter->GetParError(2) + baseline_area_sigma_error) / 2.0;
  ...
  roo_gaus_sigma =
      new RooRealVar("Roo_gaus_sigma", "Roo_gaus_sigma", std::abs(sampleSigma),
                     0.1, 3 * std::abs(sampleSigma));
  roo_gaus = new RooGaussian("Roo_gauss", "Roo_gauss", x, *roo_gaus_mean,
                             *roo_gaus_sigma);

  roo_landau_mean = new RooRealVar("Roo_landau_mean", "Roo_landau_mean", 1.0,
                                   0.0, i_histo.get_xmax());
  roo_landau_sigma =
      new RooRealVar("Roo_landau_sigma", "Roo_landau_sigma",
                     std::abs(sampleSigma), 0.1, 3 * std::abs(sampleSigma));
  roo_landau = new RooLandau("Roo_landau", "Roo_landau", x, *roo_landau_mean,
                             *roo_landau_sigma);
  ...
  RooDataHist dataHist("dataHist", i_histo.get_title().c_str(), x,
                       RooFit::Import(*i_histo.get_histo()));

  RooPlot *frame = x.frame(RooFit::Title(i_histo.get_title().c_str()));
  RooFitResult *emptyFit = lxg.fitTo(
      dataHist,
      RooFit::
          Save()); //,
                   // Normalization(histo->GetEntries(),RooAbsReal::NumEvent)
                   //);
  ...
  double range_min =
      peak -
      landau_left *
          sampleSigma; //(landau_left * sampleSigma)/1.5;//Roo_lsigma.getVal();
  double range_max =
      peak +
      landau_right *
          sampleSigma; //(landau_right * sampleSigma)/2.0;//Roo_lsigma.getVal();
  ...
  if (savePlot) {
    ...
    histo_fit =
        new TF1(Form("%s_root_fit", i_histo.get_histoName().c_str()), "landau");
    ...
    // if(i_histo.get_histo()->GetEntries() > 700 &&
    // i_histo.get_histo()->GetEntries() < 2000 )
    // i_histo.get_histo()->GetYaxis()->SetRangeUser(0, 400); else if(
    // i_histo.get_histo()->GetEntries() > 2000 )
    // i_histo.get_histo()->GetYaxis()->SetRangeUser(0, 1000); else
    // i_histo.get_histo()->GetYaxis()->SetRangeUser(0, 50);
    i_histo.get_histo()->GetYaxis()->SetRangeUser(
        0, i_histo.get_max_bin_value() * 1.5);
    i_histo.get_histo()->Draw("same");

    dataHist.plotOn(
        frame, RooFit::Name("dataHist")); //, Normalization(histo->GetEntries(),
                                          // RooAbsReal::NumEvent));
    dataHist.statOn(frame, RooFit::Layout(0.45, 0.90, 0.6));
    lxg.plotOn(frame,
               RooFit::Name("lxg")); //, Normalization(histo->GetEntries(),
                                     // RooAbsReal::NumEvent));
    lxg.paramOn(frame, RooFit::Layout(0.45, 0.90, 0.4));
    roo_landau->plotOn(
        frame, RooFit::LineStyle(kDashed),
        RooFit::LineColor(
            kOrange)); //,
                       // Normalization(histo->GetEntries(),RooAbsReal::NumEvent)
                       //);
    roo_gaus->plotOn(frame, RooFit::LineStyle(kDashed),
                     RooFit::LineColor(kPink));
    frame->Draw("same");
    ...
    img->FromPad(oCanvas);
    img->WriteImage(Form("%s_%s_lxg_RooFit.png", i_histo.get_fname().c_str(),
                         i_histo.get_title().c_str()));
    delete img;
    if (histo_fit != NULL)
      delete histo_fit;
    if (paveText != NULL)
...
FitResult Fitter::fitter_RooLanGaus(HistoPackage &i_histo, bool savePlot) {
  ...
  roo_gaus_sigma =
      new RooRealVar("Roo_gsigma", "Roo_gsigma", std::abs(sampleSigma), 0.1,
                     3 * std::abs(sampleSigma));
  roo_gaus = new RooGaussian("Roo_gauss", "Roo_gauss", x, *roo_gaus_mean,
                             *roo_gaus_sigma);

  roo_landau_mean = new RooRealVar("Roo_landau_mean", "Roo_landau_mean",
                                   sampleMean, 0.0, i_histo.get_xmax());
  roo_landau_sigma =
      new RooRealVar("Roo_landau_sigma", "Roo_landau_sigma",
                     std::abs(sampleSigma), 0.1, 3 * std::abs(sampleSigma));
  roo_landau = new RooLandau("Roo_landau", "Roo_landau", x, *roo_landau_mean,
  ...
  if (savePlot) {
    ...
    // i_histo.get_histo()->GetYaxis()->SetRangeUser(0, 400); else if(
    // i_histo.get_histo()->GetEntries() > 2000 )
    // i_histo.get_histo()->GetYaxis()->SetRangeUser(0, peak*2); else
    // i_histo.get_histo()->GetYaxis()->SetRangeUser(0, 50);
    i_histo.get_histo()->GetYaxis()->SetRangeUser(
        0, i_histo.get_max_bin_value() * 1.5);
    i_histo.get_histo()->Draw("same");
...
FitResult Fitter::fitter_fit(HistoPackage &i_histo, bool savePlot) {
  ...
  double range_min =
      this->fitter->GetParameter(1) - 5 * this->fitter->GetParameter(2);
  double range_max =
      this->fitter->GetParameter(1) + 5 * this->fitter->GetParameter(2);
  ...
  if (savePlot) {
    TCanvas *oCanvas =
        new TCanvas(Form("%s_canvas", i_histo.get_histoName().c_str()));
    oCanvas->cd();
    ...
    TImage *img = TImage::Create();
    img->FromPad(oCanvas);
    img->WriteImage(Form("%s_%s.png", i_histo.get_fname().c_str(),
                         i_histo.get_histoName().c_str()));
    delete img;
    delete oCanvas;
  }
