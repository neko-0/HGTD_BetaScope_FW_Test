...
bool BetaScope::SetInBranch(std::string branchName, std::string key) {
  ...
  try {
    this->input_branches_buffer_[this->input_branch_counter_] =
        new PrimitiveDataType_TemplateContainer<ibranchType, dtype>();
    this->input_branch_map_.insert(
        std::pair<std::string, PrimitiveDataType_BaseContainer *>(
            key, this->input_branches_buffer_[this->input_branch_counter_]));
    static_cast<PrimitiveDataType_TemplateContainer<ibranchType, dtype> *>(
        this->input_branch_map_[key])
        ->set(this->input_tree_reader_, branchName);
    ibranchType<dtype> *my_ibranch =
        BetaScope::GetInBranch<ibranchType, dtype>(key);
    if (my_ibranch) {
      ColorCout::Msg(function_name, "Branch: " + branchName + " is read.");
    }
    this->input_branch_map_index_.insert(
        std::pair<std::string, int>(key, this->input_branch_counter_));
...
ibranchType<dtype> *BetaScope::GetInBranch(std::string key) {
  return static_cast<PrimitiveDataType_TemplateContainer<ibranchType, dtype> *>(
             this->input_branch_map_[key])
      ->get();
}
