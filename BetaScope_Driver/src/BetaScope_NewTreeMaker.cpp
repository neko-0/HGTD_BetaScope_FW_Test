...
bool BetaScope::NewTreeMaker( std::string additional_branch_list )
{
    ...
    ColorCout::print(cout_prefix, "Preparing new tree. Default name: wfm", YELLOW);
    ...
    for( auto ch : this->channel )
    {
      ...
      branch_checker = makeBranch<std::vector<double>>(this->output_ttree_, Form("w%i", ch ), Form("w%i", ch ), &this->output_ttree_VecDoubleMap, this->output_ttree_VecDouble[branch_counter], branch_counter, &this->output_ttree_VecDoubleMapIndex, this->newBranchCounterKeeper );
      this->output_ttree_VecDouble[branch_counter-1]->reserve(1000000);

      branch_checker = makeBranch<std::vector<double>>(this->output_ttree_, Form("t%i", ch ), Form("t%i", ch ), &this->output_ttree_VecDoubleMap, this->output_ttree_VecDouble[branch_counter], branch_counter, &this->output_ttree_VecDoubleMapIndex, this->newBranchCounterKeeper );
      this->output_ttree_VecDouble[branch_counter-1]->reserve(1000000);
    ...
    */
    ...
    if(VERBOSITY!=0)this->output_ttree_->Print();
