...
bool BetaScope::RawTreeReader(const char *itreeName) {
  ...
  ColorCout::print(
      cout_prefix,
      "Number of events: " + std::to_string(this->input_num_event_), YELLOW);
  ...
  for (int b = 1, max = 5; b < max; b++) {
    ...
    if (isBranch) {
      br_check = BetaScope::SetInBranch<TTreeReaderArray, double>(
          Form("w%i", b), Form("w%i", b));
      br_check = BetaScope::SetInBranch<TTreeReaderArray, double>(
          Form("t%i", b), Form("t%i", b));
