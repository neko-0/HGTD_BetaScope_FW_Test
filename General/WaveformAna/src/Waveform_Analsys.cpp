...
/*
std::vector<double> WaveformAnalysis::loop_helper1( double (WaveformAnalysis::*func)(), WaveformAna<double, double> waveform, std::pair<double,unsigned int> pmaxHolder, int max)
{
  std::vector<double> fine_cfd;
  for( int step = 0; step < max; step++ ) {
      double percentage = 0.2 * step;
      fine_cfd.push_back( (this->*func)( percentage, waveform.get_v2(), waveform.get_v1(), pmaxHolder ) );
  }
  return fine_cfd;
}

std::vector<double> WaveformAnalysis::loop_helper2( double (WaveformAnalysis::*func)(), WaveformAna<double, double> waveform, std::pair<double,unsigned int> pmaxHolder, int max)
{
  std::vector<double> v;
  for(int k =0; k<max; k++) {
      v.push_back( (this->*func)( k, waveform.get_v2(), waveform.get_v1(), pmaxHolder ) );
  }
  return v;
}
*/
...
WaveformAna<double, double> WaveformAnalysis::analyze_waveform(std::vector<double> *t, std::vector<double> *w, bool limiting_search_region_OnOff, double pmaxSearchRange[2])
{
  ...
  std::pair<double,unsigned int> front_baseline_pmax = WaveformAnalysis::Find_Singal_Maximum( *w, *t, true, temp_front_searchRange );
  ...
  /*
  waveform.set_cfd( loop_helper2( &WaveformAnalysis::Rising_Edge_CFD_Time, waveform, pmaxHolder, 101) );
  waveform.set_cfd_fall( loop_helper2( &WaveformAnalysis::Falling_Edge_CFD_Time, waveform, pmaxHolder, 101) );
  waveform.set_dvdt( loop_helper2( &WaveformAnalysis::Find_Dvdt, waveform, pmaxHolder, 101) );
  waveform.set_fine_cfd( loop_helper1( &WaveformAnalysis::Rising_Edge_CFD_Time, waveform, pmaxHolder, 500) );
  waveform.set_threashold_time( loop_helper2( &WaveformAnalysis::Find_Time_At_Threshold, waveform, pmaxHolder, 2000) );
  */
