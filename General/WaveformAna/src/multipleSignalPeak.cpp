...
==============================================================================*/
std::pair<double, unsigned int> WaveformAnalysis::Find_Identical_Peak(
    std::vector<double> voltageVec, std::vector<double> timeVec,
    unsigned int StartIndex, bool limitSearchRegion, double min_search_range,
  ...
  if (StartIndex) {
    if (limitSearchRegion) {
      for (unsigned int j = StartIndex; j < npoints; j++) {
        ...
        if (min_search_range <= timeVec.at(j) &&
            timeVec.at(j) <= max_search_range) // zoom in to find the Pmax
        {
      ...
      if (StrangePeak &&
          FirstPoint) // previous sick point will be sent to here.
      {
        pmax = 10000.0;
        pmax_index = npoints;
      }
...
void WaveformAnalysis::Get_PmaxTmax_Of_Multiple_Singal(
  ...
  if (noisy_event) {
    multiple_singal_pmax_v.push_back(
        10e11); // default value if nothing is found.
    multiple_singal_tmax_v.push_back(10e11);
    indexing_v.push_back(0);
    if (!this->supressNoisy) {
      this->mu.lock();
      ColorCout::WarningMsg(
          function_name,
          "Noisy"); // if too many "Noisy", there might be a problem.
      this->mu.unlock();
    }
    ...
    if (!this->supressNoisy) {
      this->mu.lock();
      ColorCout::WarningMsg(function_name,
                            "Method fail, the output size is 0. Assigning "
                            "default value"); // if too many "Noisy", there
                                              // might be a problem.
      this->mu.unlock();
    }
  }
  ...
  for (unsigned int i = 0; i < npoints; i++) {
    if (!candidate_signal) {
      ...
      if (voltageVec.at(i) > pmax) {
        pmax = voltageVec.at(i);
        // pmax_index = i;
      } else if ((voltageVec.at(i) < assisting_threshold) &&
                 (assisting_threshold - voltageVec.at(i)) <=
                     (assisting_threshold)) {
        multiple_singal_pmax_v.push_back(pmax);
        pmax = voltageVec.at(i);
        // pmax_index = i;
