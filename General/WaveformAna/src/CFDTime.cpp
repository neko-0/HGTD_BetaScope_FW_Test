...
double WaveformAnalysis::Rising_Edge_CFD_Time(
    const double fraction, std::vector<double> voltageVec,
  ...
  for (int j = imax; j > -1; j--) {
    if (voltageVec.at(j) <= pmax * fraction / 100.0) {
      ifraction =
          j; // find index of first point before constant fraction of pulse
  ...
  if (failure)
    ...
    time_fraction =
        time_fraction +
        (timeVec.at(ifraction + 1) - timeVec.at(ifraction)) *
            (pmax * fraction / 100.0 - voltageVec.at(ifraction)) /
            (voltageVec.at(ifraction + 1) - voltageVec.at(ifraction));
...
double WaveformAnalysis::Falling_Edge_CFD_Time(
    const int fraction, std::vector<double> voltageVec,
    std::vector<double> timeVec, const std::pair<double, unsigned int> Pmax) {
  // function to calculate index of constant fraction - not truly a constant
  // fraction discriminator
  ...
  time_fraction =
      time_fraction +
      (timeVec.at(ifraction - 1) - timeVec.at(ifraction)) *
          (pmax * double(fraction) / 100.0 - voltageVec.at(ifraction)) /
          (voltageVec.at(ifraction - 1) - voltageVec.at(ifraction));
