...
double WaveformAnalysis::Find_Dvdt(const int fraction, const int ndif,
  ...
  if (ndif == 0) {
    dvdt = (voltageVec.at(ifraction + 1) - voltageVec.at(ifraction)) /
           time_difference;
  }

  else {
    dvdt = (voltageVec.at(ifraction + ndif) - voltageVec.at(ifraction - ndif)) /
           (time_difference * (ndif * 2));
  }
