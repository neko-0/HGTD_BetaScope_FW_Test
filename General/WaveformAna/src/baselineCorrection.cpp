...
)
{
  ...
  if( pmax.size() != tmax.size() ){ ColorCout::ErrorMsg( __function__, " pmax and tmax size dose not match!"); return false; }
  if( pmax.size()==0 ){ ColorCout::ErrorMsg( __function__, " pmax or tmax is empty!" ); return false; }
...
void WaveformAnalysis::SSRL_Baseline( std::vector<double> &w, std::string workerID )
{
  //TThread::Lock();
