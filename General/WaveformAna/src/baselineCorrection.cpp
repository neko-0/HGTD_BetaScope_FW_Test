...
Baseline correction using point from 0 to N.
  ...
  return: None.
=============================================================================*/
double WaveformAnalysis::Correct_Baseline(std::vector<double> &voltageVec,
                                          int ptN) {
  double mean = 0;
...
Alternate implementation of baseline correction.(2)
  ...
  return: None.
==============================================================================*/
double WaveformAnalysis::Correct_Baseline2(std::vector<double> &voltageVec,
...
Alternate implementation of baseline correction.(2)
  ...
  return: None.
==============================================================================*/
double WaveformAnalysis::Correct_Baseline3(std::vector<double> &voltageVec,
...
bool
  ...
  return: boolean.
==============================================================================*/
bool WaveformAnalysis::Correct_Baseline4(std::vector<double> &voltageVec,
                                         std::vector<double> timeVec,
...
void WaveformAnalysis::SSRL_Baseline(std::vector<double> &w,
                                     std::string workerID) {
  // TThread::Lock();
  ...
  gausFit->SetParameter(
      1, voltage_histogram->GetBinCenter(voltage_histogram->GetMaximumBin()));
...
==============================================================================*/
void WaveformAnalysis::SSRL_Baseline(std::vector<double> &w, double &RMS) {
  TThread::Lock();
  ...
  double peak =
      voltage_histogram->GetBinCenter(voltage_histogram->GetMaximumBin());
  double fwhm = 2.32 * sample_sigma;
  TF1 *gausFit =
      new TF1("gausFit", "gaus", peak - fwhm / 2.0, peak + fwhm / 2.0);
...
void WaveformAnalysis::SSRL_Dynamic_Noise_And_Baseline(
    ...
    double assisting_threshold, const double time_separation, double &RMS) {
  ...
  if (noisy_event || multiple_singal_tmax_v.size() < 2) {
    ...
    double mean =
        std::accumulate(std::begin(noise_pt), std::end(noise_pt), 0.0) /
        noise_pt.size();
    ...
    for (std::size_t i = 0, max = multiple_singal_pmax_v.size() - 1; i < max;
         i++) {
      if ((multiple_singal_tmax_v.at(i + 1) - multiple_singal_tmax_v.at(i)) >=
          time_separation) {
        std::pair<double, int> sec_pmax = std::make_pair(
            multiple_singal_pmax_v.at(i + 1), indexing_v.at(i + 1));
        double rise = Find_Rise_Time(w, t, sec_pmax, 0.1, 0.9);

        for (std::size_t j = indexing_v.at(i), jmax = indexing_v.at(i + 1);
             j < jmax; j++) {
          // ss_pt = multiple_singal_tmax_v.at(i)+5000.0;
          // ee_pt = multiple_singal_tmax_v.at(i+1)-(rise/0.9);
          if (t.at(j) >= multiple_singal_tmax_v.at(i) + 5000.0 &&
              t.at(j) <=
                  multiple_singal_tmax_v.at(i + 1) - (rise / 0.9 + 500)) {
    ...
    if (!satisfied) {
      ...
      while (true) {
        ...
        if (check == multiple_singal_tmax_v.size())
          break;
        if ((((multiple_singal_tmax_v.at(check) + selection_width) >
              start_time) &&
             ((multiple_singal_tmax_v.at(check) - selection_width) <
              start_time)) ||
            (((multiple_singal_tmax_v.at(check) + selection_width) >
              end_time) &&
             ((multiple_singal_tmax_v.at(check) - selection_width) <
    ...
    double mean = std::accumulate(noise_pt.begin(), noise_pt.end(), 0.0) /
                  noise_pt.size();
    double accum = 0.0;
