...
==============================================================================*/
double
WaveformAnalysis::Find_Pulse_Area(std::vector<double> voltageVec,
...
double WaveformAnalysis::Find_Udershoot_Area(
    std::vector<double> voltageVec, std::vector<double> timeVec,
    const std::pair<double, unsigned int> Pmax, double extendedIntRange) {
  double undershoot_area = 0.0;
  const double time_difference = timeVec.at(1) - timeVec.at(0);
...
double WaveformAnalysis::Pulse_Area_With_Linear_Interpolate_Edge(
  ...
  for (int j = imax; j > -1; j--) // find index of start of pulse
  {
    if (!found_20pmax) {
      ...
      {
        _20pmax_time =
            xlinearInter(timeVec.at(j), voltageVec.at(j), timeVec.at(j + 1),
                         voltageVec.at(j + 1), _20pmax);
        found_20pmax = true;
      }
    }
  ...
  for (unsigned int j = 0; j < iend; j++) {
    if (timeVec.at(j) >= start_time) {
      istart = j;
      pulse_area = pulse_area + ((timeVec.at(j) - start_time) / 1.0E12) *
                                    voltageVec.at(j) / 1000.0;
      break;
    }
  }
  ...
  if (integration_option.compare("Simpson") == 0) {
    ...
    for (std::size_t i = 0, max = integration_y.size(); i < max; i++) {
      if (i == 0)
        pulse_area = pulse_area + (time_difference / 3.0 / 1.0E12) *
                                      (integration_y.at(i) / 1000.0);
      else if (i == integration_y.size() - 1)
        pulse_area = pulse_area + (time_difference / 3.0 / 1.0E12) *
                                      (integration_y.at(i) / 1000.0);
      else if (i % 2 == 0)
        pulse_area = pulse_area + 2 * (time_difference / 3.0 / 1.0E12) *
                                      (integration_y.at(i) / 1000.0);
      else
        pulse_area = pulse_area + 4 * (time_difference / 3.0 / 1.0E12) *
                                      (integration_y.at(i) / 1000.0);
    }
  }
  ...
  //===========Rectrangluar=========
    ...
    for (unsigned int j = istart; j < iend; j++) {
      pulse_area =
          pulse_area + (time_difference / 1.0E12) * voltageVec.at(j) / 1000.0;
    }
  }
...
double WaveformAnalysis::Pulse_Area_With_Linear_Interpolate_Edge(
    ...
    const std::pair<double, unsigned int> Pmax, std::string integration_option,
...
double WaveformAnalysis::Pulse_Integration_with_Fixed_Window_Size(
    std::vector<double> voltageVec, std::vector<double> timeVec,
    const std::pair<double, unsigned int> Pmax, std::string integration_option,
    const double t_beforeSignal, const double t_afterSignal) {
  double pulse_area = 0.0;
  const double time_difference = timeVec.at(1) - timeVec.at(0);
  ...
  for (unsigned int i = 0; i < npoints; i++) {
    if (timeVec.at(i) >= (start_time - tRange[0]) &&
        timeVec.at(i) <= timeOfMaximum + tRange[1])
  ...
  //===========Simpson's rule=======
  if (integration_option.compare("Simpson") == 0) {
    for (std::size_t i = 0, max = integration_voltage_vector.size(); i < max;
         i++) {
      if (i == 0)
        pulse_area =
            pulse_area + (time_difference / 3.0 / 1.0E12) *
                             (integration_voltage_vector.at(i) / 1000.0);
        ...
        pulse_area =
            pulse_area + 2 * (time_difference / 3.0 / 1.0E12) *
                             (integration_voltage_vector.at(i) / 1000.0);
      else
        pulse_area =
            pulse_area + 4 * (time_difference / 3.0 / 1.0E12) *
                             (integration_voltage_vector.at(i) / 1000.0);
    }
  }
  //================================
  //===========Rectrangluar=========
    ...
    for (std::size_t j = 0, max = integration_voltage_vector.size(); j < max;
         j++) {
      pulse_area = pulse_area + (time_difference / 1.0E12) *
                                    integration_voltage_vector.at(j) / 1000.0;
    }
  }
  //================================
